...
contract TodosTest is Test {
    ...
    Todos.TodoItem public EMPTY_TODO =
        Todos.TodoItem(
            0,
            "",
            "",
    ...
    function testCreateTodos() public {
        ...
        assert(
            !sampleTodos.compareStringsbyBytes(
                todoItem.todoTitle,
                EMPTY_TODO.todoTitle
            )
        );
        assert(
            !sampleTodos.compareStringsbyBytes(
                todoItem.todoDescription,
                EMPTY_TODO.todoDescription
            )
        );
    }
    ...
    function testUpdateTodos() public {
        ...
        Todos.TodoItem memory todoItem = sampleTodos.getTodo(1);
        assert(
            sampleTodos.compareStringsbyBytes(todoItem.todoTitle, newTodoTitle)
        );
        assert(
            sampleTodos.compareStringsbyBytes(
                todoItem.todoDescription,
